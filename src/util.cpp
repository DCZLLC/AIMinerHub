...
const char * const PIVX_CONF_FILENAME = "aiminerhub.conf";
const char * const PIVX_PID_FILENAME = "aiminerhub.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";
const char * const PIVX_ACTIVE_MASTERNODE_CONF_FILENAME = "activemasternode.conf";


// AIMinerHub only features
// Masternode
...
#else
    const char* pszModule = "aiminerhub";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\aiminerhub
// Windows >= Vista: C:\Users\Username\AppData\Roaming\aiminerhub
// Mac: ~/Library/Application Support/aiminerhub
// Unix: ~/.aiminerhub
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AIMinerHub";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty aiminerhub.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override aiminerhub.conf
