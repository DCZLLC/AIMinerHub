...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("AIMinerHub client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("AIMinerHub"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("AIMinerHub"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
